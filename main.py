...
# Força a instalação das bibliotecas necessárias no ambiente do GitHub
def instalar_dependencias():
    try:
        import mediafire
        import requests
    except ImportError:
        print("🔧 Instalando bibliotecas faltantes...")
...
# Credenciais (Puxadas do GitHub Secrets)
...
def processar(arquivo_txt):
    ...
    try:
        # ID 25145 é o padrão para apps genéricos no MediaFire
        session = api.user_get_session_token(email=EMAIL, password=PASSWORD, application_id='25145')
        api.session = session
        print("🔓 Login no MediaFire realizado.")
    ...
    for url in links:
        # Pausa aleatória entre 7 e 15 segundos para evitar ser banido como bot
...
if __name__ == "__main__":
    if not EMAIL or not PASSWORD:
        print("❌ Erro: Secrets MF_EMAIL ou MF_PASSWORD não configurados no GitHub.")
    else:
        processar('links.txt')
        time.sleep(10)
        processar('links_fds.txt')import subprocess
import sys
import os
import time
import random
